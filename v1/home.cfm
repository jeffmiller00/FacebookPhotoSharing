...
<cfif IsDefined("URL.code")>
	<cfhttp method="GET" url="https://graph.facebook.com/oauth/access_token" result="rAccessToken">
		<cfhttpparam type="url" name="client_id" value="#APPLICATION.appID#" />
		<cfhttpparam type="url" name="redirect_uri" value="#SESSION.redirecturl#" />
		<cfhttpparam type="url" name="client_secret" value="#APPLICATION.secret#" />
...
<cfscript>
	...
	// URLEncodedFormat
	url_str = "https://graph.facebook.com/oauth/access_token" &
		"?client_id=" & URLEncodedFormat(APPLICATION.appID) & 
		"&redirect_uri=" & URLEncodedFormat(SESSION.redirecturl) &
		"&client_secret=" & URLEncodedFormat(APPLICATION.secret) &
		"&code=" & URLEncodedFormat(URL.code);
	adminapi.dump(url_str);
	u = CreateObject("java", "java.net.URL").Init(url_str);
	conn = u.openConnection();
	isr = CreateObject("java", "java.io.InputStreamReader").Init(conn.getInputStream());
	r = CreateObject("java", "java.io.BufferedReader").Init(isr);
	content = "";
	line = "";
	/* Would be while(line != null) in a sane language */
	while(StructKeyExists(variables, "line")) {
		line = r.readLine();
		if (StructKeyExists(variables, "line")) {
...
<cfset access_token = REReplace(rAccessToken.Filecontent,"[a-z_]*=","","one")>
<cfset access_token = REReplace(access_token,"&.*","","one")>
--->
...
<cfelse>
	...
	<cfquery name="checkReEntry" datasource="#dsn#">
		...
		FROM Footprints 
		WHERE TRUE 
			AND event_token_id = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" maxlength="10" value="#SESSION.event_token#"> 
			AND r_elat_id = <cfqueryparam cfsqltype="CF_SQL_BIGINT" value="#SESSION.relats.data#"> 
	...
	<cfif NOT checkReEntry.RecordCount>
		...
		<cfinvoke component="#fbPublisherCom#" method="GetConsumerID" returnvariable="qryConsumerID">
			<cfinvokeargument name="eventTokenID" value="#SESSION.event_token#">
		...
		<cfinvoke component="#fbPublisherCom#" method="saveConsumerData" >
			<cfinvokeargument name="consumerID" value="#SESSION.consumer_id#">
	...
	<cfelse>
		...
		<cfif consumerData["id"] NEQ FacebookID>
			...
			<cfinvoke component="#fbPublisherCom#" method="createViralConsumer" returnvariable="SESSION.event_token" >
...
<cfif albumID LTE 0>
	<cfhttp method="POST" url="https://graph.facebook.com/me/albums" result="fbNewAlbumInfo">
		<cfhttpparam type="url" name="access_token" value="#access_token#" />
	 	<cfhttpparam type="url" name="name" value="#SESSION.event_info.Brand_Name# Events" />
		<cfhttpparam type="header" name="mimetype" value="text/javascript" />
	</cfhttp>
...
<cfinvoke component="#fbPublisherCom#" method="getFBphotoID" returnvariable="photoID">
	<cfinvokeargument name="event_photo_id" value="#SESSION.event_info.event_photo_ID#">
	<cfinvokeargument name="event_token_id" value="#SESSION.event_token#">
	<cfinvokeargument name="r_elat_ID" value="#SESSION.relats.share#">
</cfinvoke>
...
--->
	...
	<cfif IsDefined("wmkInfo.watermark_file_name") AND wmkInfo.watermark_file_name NEQ "" AND SESSION.event_info.watermark_file_name EQ "blank.png">
...
<!--- <cfdump var="#Replace(SESSION.event_info.path, '/var/www/html/eshots', publicURL, 'ALL')#?q=90"> --->
	...
	<cfhttp method="POST" url="https://graph.facebook.com/#albumID#/photos" result="fbPhotoInfo">
		<cfhttpparam type="url" name="access_token" value="#access_token#" />
	 	<cfhttpparam type="file" name="source" file="#SESSION.event_info.path#" />
		<cfhttpparam type="header" name="mimetype" value="text/javascript" />
	</cfhttp>
	...
	<cfif Find("200",fbPhotoInfo.Responseheader.Status_Code)>
		...
		<cfquery name="qryRFPinsert" datasource="#dsn#">
			INSERT INTO efn.R_Footprint_Photo (footprint_ID, event_photo_ID) VALUES (#footprint_id#, #SESSION.event_info.event_photo_ID#);
		</cfquery>
